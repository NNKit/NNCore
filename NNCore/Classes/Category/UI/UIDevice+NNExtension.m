...
@implementation UIDevice (NNExtension)
...
static uint64_t yy_net_counter_get_by_type(yy_net_interface_counter *counter, NNetworkTrafficType type) {
    uint64_t bytes = 0;
    if (type & NNetworkTrafficTypeWWANSent) bytes += counter->pdp_ip_out;
    if (type & NNetworkTrafficTypeWWANReceived) bytes += counter->pdp_ip_in;
    if (type & NNetworkTrafficTypeWiFiSent) bytes += counter->en_out;
    if (type & NNetworkTrafficTypeWiFiReceived) bytes += counter->en_in;
    if (type & NNetworkTrafficTypeAWDLSent) bytes += counter->awdl_out;
    if (type & NNetworkTrafficTypeAWDLReceived) bytes += counter->awdl_in;
    return bytes;
}
